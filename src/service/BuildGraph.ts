...
export const createBuildPlan = (repos: Repo[], changedModuleNames: string[]): PhasedPlan[] => {
    ...
    const queue: string[] = Array.from(changedRepos).filter(repoName => indegree.get(repoName) === 0);
    ...
    while (queue.length) {
        ...
        for (let i = 0; i < size; i++) {
            ...
            if (processedRepos.has(currentRepoName)) {
                continue;
            }

            currentPhaseRepos.push(currentRepoName);
            processedRepos.add(currentRepoName);

            repoDependencies.get(currentRepoName)?.forEach(depRepoName => {
